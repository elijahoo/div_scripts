...
def convert_lagerort_to_artikel
  ...
  lagerorte.each do |lager|
    a = create_lagerort_artikel(lager.nummer)
...
def create_regale()
  ...
  regale = {}
  Kartei::Lgrbstnd.tree_search(:baum => 0, :von => '', :bis => "\376\376") do |id, suchstring|
    key = [suchstring['lgr1tck'], suchstring['regal']].join('~~~')
    if not suchstring['regal'].empty? and not regale.has_key?(key)
      regale.store(key, [suchstring['regal'], suchstring['lgr1tck']])
    end
  end

  regale.values.each do |regal, lagerort_ticket|
...
def create_lagerort_artikel(lager_nummer, status = 0)
    ...
    file = File.open( "#{EXP}/#{mandant.dir_name}/#{krtname}.exp", 'r').each do |line|
      ...
      if nr == '.'
        ...
        unless lagerort_artikel
          ...
          lagerort_artikel = create_lagerort_artikel(lager_nummer, 2)
